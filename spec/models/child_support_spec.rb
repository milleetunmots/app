...
RSpec.describe ChildSupport, type: :model do
  ...
  let(:first_parent) { FactoryBot.create(:parent, postal_code: 75006) }
  let(:second_parent) { FactoryBot.create(:parent, postal_code: 99999) }
  let(:third_parent) { FactoryBot.create(:parent, postal_code: 88888) }
  let(:fourth_parent) { FactoryBot.create(:parent, postal_code: 55555) }

  let(:admin_user) { FactoryBot.create(:admin_user) }

  let(:first_child_support) { FactoryBot.create(:child_support, supporter: admin_user )}
  let(:second_child_support) { FactoryBot.create(:child_support )}
  let(:third_child_support) { FactoryBot.create(:child_support )}

  let(:first_family) { FactoryBot.create(:family, parent1: first_parent, parent2: third_parent, child_support: first_child_support)}
  let(:second_family) { FactoryBot.create(:family, parent1: second_parent, parent2: fourth_parent, child_support: second_child_support)}
  let(:third_family) { FactoryBot.create(:family, parent1: first_parent, child_support: third_child_support)}
