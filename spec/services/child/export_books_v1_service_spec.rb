...
RSpec.describe Child::ExportBooksV1Service do
  describe "find_children_lists" do
    ...
    let_it_be(:first_group_between_0_and_12_children, reload: true) { FactoryBot.create_list(:child, 3, birthdate: Time.zone.today.prev_month(5), group: first_group, group_status: 'active')}
    let_it_be(:second_group_between_0_and_12_children, reload: true) { FactoryBot.create_list(:child, 4, birthdate: Time.zone.today.prev_month(5), group: second_group, group_status: 'active')}
    let_it_be(:third_group_between_0_and_12_children, reload: true) { FactoryBot.create_list(:child, 2, birthdate: Time.zone.today.prev_month(3), group: third_group, group_status: 'active')}
    let_it_be(:second_group_between_12_and_24_children, reload: true) { FactoryBot.create_list(:child, 4, birthdate: Time.zone.today.prev_month(15), group: second_group, group_status: 'active')}
    let_it_be(:third_group_between_12_and_24_children, reload: true) { FactoryBot.create_list(:child, 2, birthdate: Time.zone.today.prev_month(14), group: third_group, group_status: 'active')}
    let_it_be(:third_group_more_than_24_children, reload: true) { FactoryBot.create_list(:child, 2, birthdate: Time.zone.today.prev_month(25), group: third_group, group_status: 'active')}
