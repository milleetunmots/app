class TypeformController < ApplicationController
  ...
  MIDWAY_TYPEFORM_ID = ENV['MIDWAY_TYPEFORM_ID']
  CALL_ZERO_GOALS_TYPEFORM_ID = ENV['CALL_ZERO_GOALS_TYPEFORM_ID']
  CALL_THREE_SPEAKING_TYPEFORM_ID = ENV['CALL_THREE_SPEAKING_TYPEFORM_ID']
  CALL_THREE_OBSERVING_TYPEFORM_ID = ENV['CALL_THREE_OBSERVING_TYPEFORM_ID']
  INITIAL_TYPEFORM_ID = ENV['INITIAL_TYPEFORM_ID']
  UPDATING_ADDRESS_TYPEFORM_ID = ENV['UPDATING_ADDRESS_TYPEFORM_ID']
  TYPEFORM_IDS = [
    MIDWAY_TYPEFORM_ID,
  ...
  def webhooks
    if params[:form_response][:form_id].in? TYPEFORM_IDS
      ...
      Rollbar.error('Typeform service error', service: service.class.to_s, errors: service.errors) unless service.errors.empty?
    else
      Rollbar.error('Typeform with unknown id', typeform_id: params[:form_response][:form_id])
    end
