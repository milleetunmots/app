class ChildrenSupportModule
  class VerifyChosenModulesTaskJob < ApplicationJob
    def perform(group_id)
      ...
      logistics_team_members.each { |ltm| Task.create(assignee_id: ltm.id, title: "Compléter les modules pour la cohorte \"#{group.name}\"", description: description_text, due_date: Time.zone.today ) }
