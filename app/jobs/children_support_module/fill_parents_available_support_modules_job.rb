  ...
  class FillParentsAvailableSupportModulesJob < ApplicationJob
    ...
    def perform(group_id, second_support_module)
      ...
      group.children.each do |child|
        support_module_ids = order_available_support_modules(child)
        if second_support_module
          filling_child_support(child, support_module_ids)
        else
          filling_child_support(child, support_module_ids.first(3))
        end
      end
    end
    ...
    def filling_child_support(child, ids)
      child.child_support.parent1_available_support_module_list = []
      child.child_support.parent2_available_support_module_list = []
      ids.each do |id|
        child.child_support.parent1_available_support_module_list << id.to_s
        child.child_support.parent2_available_support_module_list << id.to_s
