    ...
    unless current_admin_user.caller?
      dropdown_menu 'Logistique' do
        item 'Nettoyer la fiche de suivi', [:clean_child_support, :admin, resource],
             { data: { confirm: 'Êtes-vous sûr de vouloir nettoyer la fiche de suivi ? Cette action est Irréversible, toutes les informations des appels vont être vidées et reportées danns les notes' } }
      end
    end
  end
