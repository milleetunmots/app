class Parent::ProgramCafSubscriptionReminderService
  ...
  MESSAGE_V1 = "1001mots : Bonjour, Vous avez commencé votre inscription à 1001mots. Mais il vous reste une dernière étape : prendre rdv pour un appel avec une accompagnante 1001mots.\nPrenez rendez-vous dès aujourd'hui et commencez l'accompagnement dès la semaine prochaine ! https://form.typeform.com/to/w9H4beIF#child_support_id=xxxxx&current_child_name=xxxxx&parent1_last_name=xxxxx&email=xxxxx&current_child_months=xxxxx&sc=xxxxx\nÀ très vite,\nL'équipe 1001mots".freeze
  ...
  def call
    @children.find_each do |child|
      ...
      @message = @message.gsub('child_support_id=xxxxx', "child_support_id=#{@parent.id}")
      ...
      @message = @message.gsub('sc=xxxxx', "sc=#{child.paernt1.security_code}")
