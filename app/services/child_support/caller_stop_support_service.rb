class ChildSupport::CallerStopSupportService
	...
	VARIABLES = {
			program: { tag: 'arrêt appelante - programme', sms: PROGRAM_SMS, url: SMS_LINK },
			popi: { tag: 'arrêt appelante - popi', sms: POPI_SMS, url: SMS_LINK },
			professional: { tag: 'arrêt appelante - pro de santé', sms: PROFESSIONAL_SMS, url: PROFESSIONAL_SMS_LINK },
			problematic_case: { tag: 'arrêt appelante - problèmes', sms: PROBLEMATIC_CASE_SMS, url: nil }
		}.freeze
	...
	def add_stop_support_informations
		@child_support.support_stop_caller_id = @supporter.id
		@child_support.support_stop_date = DateTime.now
		@child_support.important_information = "#{@child_support.important_information}\nFamille arretée le #{@child_support.support_stop_date.strftime("%d/%m/%Y")} par #{@supporter.name}"
