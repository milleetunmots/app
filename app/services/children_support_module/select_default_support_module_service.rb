class ChildrenSupportModule
  class SelectDefaultSupportModuleService
    ...
    SPECIFIC_DEFAULT_SUPPORT_MODULE_NAME_START = 'Jouer avec mon'.freeze
    ...
    def call
      ...
      return self unless missing_support_modules_for_current_children?

      retry_assign_default_support_module
      return self unless missing_support_modules_for_current_children?

      assign_specific_default_support_module
      return self unless missing_support_modules?
    ...
    def assign_specific_default_support_module
      five_to_eleven_specific_default_support_module = SupportModule.where('name ILIKE ?', "#{SPECIFIC_DEFAULT_SUPPORT_MODULE_NAME_START}%").where("'#{SupportModule::FIVE_TO_ELEVEN}' = ANY (age_ranges)").first
      twelve_to_seventeen_specific_default_support_module = SupportModule.where('name ILIKE ?', "#{SPECIFIC_DEFAULT_SUPPORT_MODULE_NAME_START}%").where("'#{SupportModule::TWELVE_TO_SEVENTEEN}' = ANY (age_ranges)").first
      eighteen_to_twenty_three_specific_default_support_module = SupportModule.where('name ILIKE ?', "#{SPECIFIC_DEFAULT_SUPPORT_MODULE_NAME_START}%").where("'#{SupportModule::EIGHTEEN_TO_TWENTY_THREE}' = ANY (age_ranges)").first
      twenty_four_to_twenty_nine_specific_default_support_module = SupportModule.where('name ILIKE ?', "#{SPECIFIC_DEFAULT_SUPPORT_MODULE_NAME_START}%").where("'#{SupportModule::TWENTY_FOUR_TO_TWENTY_NINE}' = ANY (age_ranges)").first
      thirty_to_thirty_five_specific_default_support_module = SupportModule.where('name ILIKE ?', "#{SPECIFIC_DEFAULT_SUPPORT_MODULE_NAME_START}%").where("'#{SupportModule::THIRTY_TO_THIRTY_FIVE}' = ANY (age_ranges)").first
      ...
      @missing_support_modules.each do |children_support_module|
        ...
        children_support_module.update(support_module: support_module)
      end
    end

    def missing_support_modules_for_current_children?
      @missing_support_modules = ChildrenSupportModule.where(support_module: nil, child_id: @active_current_children, module_index: @current_support_module_index)
      @missing_support_modules.any?
    end

    def missing_support_modules?
      @missing_support_modules = ChildrenSupportModule.where(support_module: nil, child_id: @active_children, module_index: @current_support_module_index)
      @missing_support_modules.any?
    end
  end
end
