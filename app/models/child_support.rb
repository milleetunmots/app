...
class ChildSupport < ApplicationRecord
  ...
  def self.previous_calls_ok_or_unfinished_before(call_index)
    ...
    conditions = (0...call_index).map do |i|
      arel_table["call#{i}_status"].eq(ChildSupport.human_attribute_name("call_status.1_ok"))
                                   .or(
                                     arel_table["call#{i}_status"].eq(ChildSupport.human_attribute_name("call_status.5_unfinished"))
                                   )
    end
