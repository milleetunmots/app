namespace :groups do
  ...
  task add_support_modules_sent_dates: :environment do
    Group.where(is_programmed: true).where.not(started_at: nil).find_each do |group|
      update_group_support_module_sent_date(0, group.started_at, group)
      update_group_support_module_sent_date(1, group.started_at + Group::ProgramService::MODULE_ZERO_DURATION, group)
      ...
      (3..group.support_modules_count).each do |module_index|
        ...
        update_group_support_module_sent_date(module_index - 1, program_module_date, group)
      end
    end
  end
